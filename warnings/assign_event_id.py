...
def main():
    ...
    for row in cursor:
        ...
        if cursor2.rowcount == 0:
            print(
                "Assign 1? %s %s %s %s" % (wfo, phenomena, significance, ugc)
            )
            cursor2.execute(
                """UPDATE """
                + table
                + """ SET
            eventid = 1 WHERE oid = %s """,
                (oid,),
            )
            continue
        events = []
        ...
        for i in range(len(events) - 1):
            if issues[i] < issue and issues[i + 1] > issue:
                ...
                print(
                    "HERE %s %s %s %s %s"
                    % (oid, issue, issues[i], issues[i + 1], events[i])
                )
                continue

        # see if this is a DUP!
        cursor2.execute(
            f"SELECT count(*) from {table} WHERE oid != %s and wfo = %s and "
            "ugc = %s and phenomena = %s and significance = %s and issue = %s",
            (oid, wfo, ugc, phenomena, significance, issue),
        )
        ...
        print(
            "Fail? %s %s %s %s %s %s"
            % (oid, issue, issues[0], events[0], issues[-1], events[-1])
        )
        continue
