...
s = mx.DateTime.DateTime(2004,1,1)
...
tickets = mydb.query("SELECT * from tt_base WHERE portfolio = 'iarwis' \
  and subject = 'Site Offline' and author = 'mesonet' \
  and entered > '%s' and entered < '%s' \
  ORDER by id ASC" % (s.strftime("%Y-%m-%d"), e.strftime("%Y-%m-%d") ) ).dictresult()
...
for station in st.ids:
  if station != "RDES" and station != "RIOW":
    if (Odur[station] > 0):
