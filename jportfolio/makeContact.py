...
for station in st.ids:
  sql = "SELECT * from iem_site_contacts WHERE portfolio = '%s' and \
         email = '%s' and s_mid = '%s'" % (portfolio, email, station)
  rs = mydb.query(sql).dictresult()

  if len(rs) == 0:
    sql = "INSERT into iem_site_contacts (portfolio, s_mid, \
           name, phone, email) VALUES ('%s', '%s', \
           '%s', ' ', '%s')" % (portfolio, station, name, email)
