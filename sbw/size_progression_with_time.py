...
def do_year(pgconn, year):
    ...
    """)
    for row in cursor:
        idx0 = int((row['s_pb'] - row['polygon_begin']).total_seconds() / 60.)
        idx1 = idx0 + int(
            (row['s_pe'] - row['s_pb']).total_seconds() / 60.)
...
def main():
    ...
    fig.text(0.5, 0.92, ("NWS Storm-Based Tornado Warning Size Progression\n"
                         "All Warnings Equally Weighted"), ha='center',
             fontsize=16)
    ...
    for year in tqdm(range(2008, 2018)):
        ...
        v30.append(res[30])
        v45.append(res[45])
        ax.plot(np.arange(120), res, next(linecycler), label="%s" % (year, ),
                lw=2)
    ax.legend(ncol=5)
