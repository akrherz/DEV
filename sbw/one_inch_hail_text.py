...
def main():
    ...
    # Query for warnings
    rs = postgis.query(
        "SELECT report, wfo, issue, expire, astext(geom) as txt from \
        warnings_2007 WHERE wfo IN ('FSD','OAX','DSM','DVN','ARX') and \
        gtype = 'P' and significance = 'W' and phenomena = 'SV'"
    ).dictresult()
    ...
    for i in range(len(rs)):
        ...
        tokens = re.findall(
            "CAPABLE OF PRODUCING ([34/A-Z ]+)\s+(INCH|SIZE)\s+HAIL", r
        )
        if len(tokens) == 0:
            print(rs[i]["report"])
        if len(tokens) > 0:
            sz = tokens[0][0]
            if sz not in mentioned:
                mentioned[sz] = 0
                verifsz[sz] = 0
            mentioned[sz] += 1
        else:
        ...
        if (
            not verif.has_key("D")
            and not verif.has_key("G")
            and not verif.has_key("L")
            and verif.has_key("S")
        ):
            small_only += 1
        if verif.has_key("L"):
