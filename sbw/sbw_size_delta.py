...
rs = postgis.query(
    """SELECT distinct wfo, eventid, significance, phenomena,
     issue, expire from sbw_2008 WHERE phenomena = 'TO'"""
).dictresult()
# Create data array
...
for i in range(len(rs)):
    ...
    for j in range(len(rs2)):
        psts = mx.DateTime.strptime(
            rs2[j]["polygon_begin"][:16], "%Y-%m-%d %H:%M"
        )
        pets = mx.DateTime.strptime(
            rs2[j]["polygon_end"][:16], "%Y-%m-%d %H:%M"
        )
        psz = rs2[j]["area"]
