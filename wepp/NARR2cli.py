...
while (now < ets):
  ...
  tmpk = NetCDF.NetCDFFile("/mnt/nrel/cjames/NARR/air.2m.%s.nc" % (now.year,)).variables['air'][:,y,x]

  # dpt:add_offset = 492.66f ;
  # dpt:scale_factor = 0.01f ;
  dwpk = NetCDF.NetCDFFile("/mnt/nrel/cjames/NARR/dpt.2m.%s.nc" % (now.year,)).variables['dpt'][:,y,x]

  #  Watts/per/meter sq
  #  dswrf:add_offset = 3041.6f ;
  #  dswrf:scale_factor = 0.1f ;
  solar = NetCDF.NetCDFFile("/mnt/nrel/cjames/NARR/dswrf.%s.nc" % (now.year,)).variables['dswrf'][:,y,x]

  #  apcp:add_offset = 50.f ;
  #  apcp:scale_factor = 0.001526019f ;
  precip = NetCDF.NetCDFFile("/mnt/nrel/cjames/NARR/apcp.%s.nc" % (now.year,)).variables['apcp'][:,y,x]
  ...
  while (now < nextyear):
    ...
    if (rsum < 5):
      ptxt = ""
      pticks = 0
    o.write("%s\t%s\t%s\t%s\t%3.1f\t%3.1f\t%4.0f\t%4.1f\t%s\t%4.1f\n%s" % \
      (now.day, now.month, now.year, pticks, highc, \
       lowc, langley, wmps, drct, dwpc, ptxt))
    now += interval
