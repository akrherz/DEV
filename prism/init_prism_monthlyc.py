"""Create and populate the Monthly PRISM climatology netCDF."""
...
def populate_file():
    ...
    """
    fn = BASEDIR / "prism_monthlyc.nc"
    nc = ncopen(fn, "a")
    for varname in ["ppt", "tmax", "tmin"]:
        for month in range(1, 13):
            tiff_fn = (
                BASEDIR
                / f"prism_{varname}_us_30s_2020{month:02.0f}_avg_30y.tif"
            )
...
def init_file():
    ...
    nc = ncopen(fn, "w")
    nc.title = "PRISM Monthly Climatology"
    ...
    tm = nc.createVariable("month", float, ("month",))
    tm.units = "Month of the Year"
    tm[:] = np.arange(1, 13)

    # Note the reduced resolution than daily data, max value is ~1300 in prism
    p01d = nc.createVariable(
        "ppt", np.ushort, ("month", "lat", "lon"), fill_value=65535
    )
    # ~1650
    p01d.scale_factor = 0.025
    p01d.add_offset = 0.0
    p01d.units = "mm"
    p01d.long_name = "Precipitation"
    p01d.standard_name = "Precipitation"
    p01d.coordinates = "lon lat"
    p01d.description = "Precipitation accumulation for the month"

    high = nc.createVariable(
        "tmax", np.uint8, ("month", "lat", "lon"), fill_value=255
    )
    high.scale_factor = 0.5
    high.add_offset = -60.0
    high.units = "C"
    high.long_name = "2m Air Temperature Monthly High"
    high.standard_name = "2m Air Temperature"
    high.coordinates = "lon lat"

    low = nc.createVariable(
        "tmin", np.uint8, ("month", "lat", "lon"), fill_value=255
    )
    low.scale_factor = 0.5
    low.add_offset = -60.0
    low.units = "C"
    low.long_name = "2m Air Temperature Monthly High"
