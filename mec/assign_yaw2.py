...
def compute_correction(unitnumber, turbineid):
    ...
    (counts, xedges, yedges, img) = ax[0].hist2d(
        asos, turbine, bins=36, cmin=5
    )
    ...
    if cor_stddev > 30:  # arb threshold
        ...
        basis = 180.0
        turbine2 = turbine + 180.0
        turbine2 = np.where(turbine2 > 360, turbine2 - 360.0, turbine2)
    ...
    (counts, xedges, yedges, img) = ax[2].hist2d(
        asos, turbine2, bins=36, cmin=1
    )
    ...
    diff = np.where(diff < -180, diff + 360.0, diff)
