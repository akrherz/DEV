...
ds = drv.Create('%s.img' % (CROPS[LEVELINDEX],),
                len(nc.dimensions['longitude']),
                len(nc.dimensions['latitude']), 1,
                gdal.GDT_Float32, options=['COMPRESS=YES'])

# Manual projection hack here, hold yer nose
srs = ('GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",'
...
ds.SetGeoTransform([lons[0], dx, 0,
                    lats[0], 0, 0 - dy])
print(('We wrote data with min and max: %s'
