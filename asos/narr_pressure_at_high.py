...
cursor.execute(
    "SELECT day, day + '1 day'::interval, high from alldata_ia "
    "where station = 'IA2203' and high > 99 and month = 8 and "
    "day > '1979-01-01' ORDER by day"
)
xs = []
ys = []
xx = []
for row in cursor:
    uri = row[0].strftime(
        "http://nomads.ncdc.noaa.gov/thredds/ncss/grid/"
...
m = MapPlot(
    ...
    subtitle=(
        "12 UTC analysis for %s days in August where Des Moines hit "
        "100+$^\circ$F High"
    )
    % (cursor.rowcount,),
)
