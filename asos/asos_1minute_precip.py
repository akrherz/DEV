...
def main():
    ...
    sz = int((ets - sts).days * 1440 + (ets - sts).seconds / 60.0) + 1
    ...
    for i in range(1, sz):
        rate1[i] = (prec[i] - prec[i - 1]) * 60.0
    for i in range(15, sz):
        rate15[i] = (prec[i] - prec[i - 15]) * 4.0
    ...
    print(
        "MaxI: %s, rate: %s, window: %s-%s"
        % (maxi, rate1[maxi], maxwindowi, maxwindowi + 10)
    )
    ...
    ax.set_title(
        (
            "17 July 2018 Washington DC, Reagan National (DCA)\n"
