...
def main():
    ...
    for sid in tqdm(nt.sts):
        ...
        """, postgis, params=(nt.sts[sid]['lon'], nt.sts[sid]['lat']))
        if df.empty:
            continue
        ...
        for i, row in df.iterrows():
            j = i
            while (j < obs and
                    (df.valid[j] - row['valid']).total_seconds() < (3 * 3600)):
                j += 1
    ...
    fig.text(
        .5, .95, "Iowa ASOS/AWOS 25 November 2018 Blizzard Criteria",
    ...
    ax1.scatter(
        bs_vsby, np.array(bs_sknt) * 1.15, marker='+', color='r',
        label='Best Wind')
    ax1.scatter(
        bv_vsby, np.array(bv_sknt) * 1.15, marker='o', facecolor='w',
        edgecolor='b', label="Best Vis")
    ax1.set_xscale('log')
    ax1.legend()

    ax2.scatter(
        abs_vsby, np.array(abs_sknt) * 1.15, marker='+', color='r',
        label='Best Wind')
    ax2.scatter(
        abv_vsby, np.array(abv_sknt) * 1.15, marker='o', facecolor='w',
        edgecolor='b', label='Best Vis')
    ax2.set_xscale('log')
    ax2.legend()

    ax3.scatter(
        mbs_vsby, np.array(mbs_sknt) * 1.15, marker='+', color='r',
        label='Best Wind')
    ax3.scatter(
        mbv_vsby, np.array(mbv_sknt) * 1.15, marker='o', facecolor='w',
        edgecolor='b', label='Best Vis')
    ax3.set_xscale('log')
    ax3.legend()
