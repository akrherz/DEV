...
def main():
    ...
    """, pgconn, params=('MXO', ), index_col='valid')
    xticks = []
    xticklabels = []
    ...
    ax.bar(df.index.values, speed(df['gust'], 'KT').value('MPH'),
           width=10/1440., color='red')
    ax.bar(df.index.values, speed(df['sknt'], 'KT').value('MPH'),
           width=10/1440., color='tan')
    ax.set_ylabel("Wind Speed (tan) & Gust (red) [mph]")
    ax.grid(True, zorder=5)
    ax.set_ylim(0, 60)

    ax2 = ax.twinx()
    ax2.plot(df.index.values, pressure(df['alti'], 'IN').value('MB'),
             color='g', lw=2)
