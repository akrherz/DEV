...
def main():
    ...
    daily["mixingratio"] = meteorology.mixing_ratio(
        temperature(daily["low"].values, "F")
    ).value("KG/KG")
    daily["vapor_pressure"] = mcalc.vapor_pressure(
        1000.0 * units.mbar, daily["mixingratio"].values * units("kg/kg")
    ).to(units("kPa"))
    daily["saturation_mixingratio"] = meteorology.mixing_ratio(
        temperature(daily["high"].values, "F")
    ).value("KG/KG")
    ...
    for date, gdf in df.groupby(df.index.date):
        ...
        if date.month == 7:
            print(
                "%s %.3f %.3f"
                % (date, daily.at[date, "vpd"], gdf["vpd"].max())
            )
            rows.append(
                dict(
                    date=date,
                    daily=daily.at[date, "vpd"],
                    hourly=gdf["vpd"].max(),
                )
            )
