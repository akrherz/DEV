...
def two():
    ...
    for sid in sids:
        print("-------------------------------------------------------------")
        print(
            (" Data Since: %s Station: [%s] %s")
            % (nt.sts[sid]["archive_begin"].year, sid, nt.sts[sid]["name"])
        )
        for hrs in range(12, 24 * 12 + 1, 24):
            uri = (
                "https://mesonet.agron.iastate.edu/plotting/auto/plot/42/"
            ...
            for _, row in mysort.iterrows():
                ets = datetime.datetime.strptime(
                    row["end"][:16], "%Y-%m-%d %H:%M"
                )
                sts = datetime.datetime.strptime(
                    row["start"][:16], "%Y-%m-%d %H:%M"
                )
                if sts.year == 2015:
                    found2015 = True
...
def one():
    ...
    for sid in ids:
        ...
        for _, row in enumerate(cursor):
            hdx = heatindex(
                temperature(row[1], "F"), temperature(row[2], "F")
            ).value("F")
