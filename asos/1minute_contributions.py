...
df["total_ratio"] = df["total"] / df["total"].sum() * 100.0
df["count_ratio"] = df["count"] / df["count"].sum() * 100.0
...
ax[1].grid(True)
ax[1].text(
    0.7, 0.7, "Same Data As Above\nJust Log Scale", transform=ax[1].transAxes
)
