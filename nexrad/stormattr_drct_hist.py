...
def main():
    ...
    alldf['freq'] = alldf['count'] / alldf['count'].sum() * 100.
    yestdf = read_sql("""
    ...
    """, pgconn, index_col=None)
    yestdf['freq'] = yestdf['count'] / yestdf['count'].sum() * 100.

    (fig, [ax1, ax2]) = plt.subplots(2, 1, sharex=True)
    ax1.bar(alldf['bindrct'] * 15. + 7.5, alldf['freq'], width=15, alpha=0.5,
            fc='r', label='2000-2017')
    ax1.set_xticks(range(0, 361, 45))
    ax1.set_xticklabels(["N", "NE", "E", "SE", "S", "SW", "W", "NW", "N"])
    ax1.grid(True)
    ax1.legend()
    ax1.set_ylabel("Frequency [%]")
    ax1.set_title(("Des Moines NEXRAD Storm Attribute Direction Frequency"
                   "\n2000-2017 for storms traveling at least 5 knots speed"
                   ))
    ax2.bar(yestdf['bindrct'] * 15. + 7.5, yestdf['freq'], width=15, alpha=0.5,
            fc='b', label='4 Jul 2017')
