...
def main():
    ...
    t = []
    ccursor.execute(
        """SELECT day, (high+low)/2.0 from alldata_ia
    ...
    for yr in range(1950, 2013):
        ...
        if elnino[yr - 1950] < 0:
            c = "b"
        ax[1].text(
            yr, death[yr - 1901], "%.1f" % (elnino2[yr - 1950],), color=c
        )
    ax[1].grid(True)
    ticks = []
    labels = []
    ...
    ax[2].set_title(
        (
            "Date of Yearly 'Golden Cross' "
            "(50DMA surpasses 200DMA), DJF ENSO 3.4"
        )
    )
    ...
    for yr in range(1950, 2013):
        ...
        if elnino[yr - 1950] < 0:
            c = "b"
        ax[2].text(
            yr, golden[yr - 1901], "%.1f" % (elnino[yr - 1950],), color=c
        )
    ax[2].grid(True)
    ticks = []
    labels = []
