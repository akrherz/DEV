...
def main():
    ...
    m = MapPlot(
        title="Fall 2016 Minimum Temperature Reports",
        axisbg="white",
        subtitle=(
            "Based on NWS Cooperative Observer Data, " "thru 27 Dec 2016"
        ),
        sector="midwest",
    )
    ...
    m.map.scatter(
        x, y, marker="o", color="g", s=50, zorder=1, label="10 to 32$^\circ$F"
    )
    x, y = m.map(tuple(x2.x), tuple(x2.y))
    m.map.scatter(
        x, y, marker="s", color="b", zorder=1, s=50, label="-10 to 10$^\circ$F"
    )
