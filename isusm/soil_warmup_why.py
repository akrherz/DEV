...
def magic(ax, xvals, yvals):
    """You can do magic"""
    slope, intercept, r_value, p_value, std_err = stats.linregress(xvals,
                                                                   yvals)
    ax.scatter(xvals, yvals)
    xpts = np.array(ax.get_xlim())
    ax.plot(xpts, slope * xpts + intercept, color='r')
    ax.text(1.1, 0.01, "Slope: %.1f R$^2$= %.1f" % (slope, r_value ** 2),
            ha='right', bbox=dict(color='w'), transform=ax.transAxes)
...
def main():
    ...
    df['avgt'] = (df['high'] + df['low']) / 2.
    df['lag_avgt'] = (df['lag_high'] + df['lag_low']) / 2.
    ...
    fig.text(0.5, 0.95, ("Ames ISU Ag Climate Station 1986-2014\n"
