...
def parse_colorramp():
    ...
    for level in xml.getElementsByTagName("Level"):
        val = level.getAttribute("upperValue") or level.getAttribute(
            "lowerValue"
        )
...
def compute_xy(scan, lon, lat):
    ...
    o = open(
        scan.strftime(
            "/mesonet/ARCHIVE/data/%Y/%m/%d/GIS/ridge/DMX/N0Q/DMX_N0Q_%Y%m%d%H%M.wld"
        ),
        "r",
    )
    lines = o.readlines()
...
def get_value(scan, x, y):
    ...
    image = gdal.Open(
        scan.strftime(
            "/mesonet/ARCHIVE/data/%Y/%m/%d/GIS/ridge/DMX/N0Q/DMX_N0Q_%Y%m%d%H%M.png"
        ),
        0,
    )
