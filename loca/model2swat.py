...
def main(argv):
    ...
    for year in years:
        ...
        tasmax_nc = netCDF4.Dataset(
            (
                "%s/tasmax/tasmax_day_%s_%s_r1i1p1"
        ...
        tasmin_nc = netCDF4.Dataset(
            (
                "%s/tasmin/tasmin_day_%s_%s_r1i1p1"
                "_%.0f0101-%.0f1231.LOCA_"
                "2016-04-02.16th.nc"
            )
            % (basedir, model, rcp, year, year)
        ...
        for i in tqdm(range(timesz), desc=str(year)):
            ...
            tasmax = np.flipud(
                temperature(tasmax_nc.variables["tasmax"][i, :, :], "K").value(
                    "C"
                )
            )
            tasmin = np.flipud(
                temperature(tasmin_nc.variables["tasmin"][i, :, :], "K").value(
                    "C"
                )
            )
