...
geoms = [
    ...
    "geom",
    "ST_Simplify(geom,0.1)",
    "ST_Simplify(geom,0.01)",
...
for row in range(2):
    for col in range(3):
        ...
        m = Basemap(
            ...
            llcrnrlon=xmin,
            lon_0=(xmax + xmin) / 2.0,
            lat_0=(ymax + ymin) / 2.0 - 5,
            lat_1=(ymax + ymin) / 2.0,
            lat_2=(ymax + ymin) / 2.0 + 5,
            resolution="l",
            fix_aspect=True,
            ax=ax,
        )
        ...
        data = source.ExecuteSQL(
            "select %s from nws_ugc where ugc = '%s'" % (g, UGC)
        )
        ...
        while 1:
            ...
            for polygon in geom:
                ...
                x, y = m(a[:, 0], a[:, 1])
                a = zip(x, y)
                p = Polygon(a, fc="r", ec="k", zorder=2, lw=0.5)
                patches.append(p)

        ax.set_title(g, fontsize=8)
        ax.set_xlabel(
            "Geometry size: %.3f KB" % (sys.getsizeof(bindata) / 1024.0,),
            fontsize=8,
        )
