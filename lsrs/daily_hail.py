...
def main():
    ...
    pcursor.execute(
        ...
        extract(hour from valid) as hr, count(*) from (select distinct valid,
        city, magnitude from lsrs
    ...
    ax.grid(True)
    res = ax.imshow(
        obs, aspect="auto", rasterized=True, interpolation="nearest"
    )
    fig.colorbar(res)
    ax.set_ylim(-0.5, 7.5)
    ...
    res = ax.imshow(
        hrs, aspect="auto", rasterized=True, interpolation="nearest"
    )
    ax.grid(True)
