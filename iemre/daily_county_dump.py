...
def main():
    ...
    """, pgconn, index_col='fips', geom_col='the_geom')
    ...
    for year in tqdm(range(1950, 2018)):
        ...
        for tstep in range(nc.variables['time'].size):
            sdate = (datetime.date(year, 1, 1) +
                     datetime.timedelta(days=tstep)).strftime("%Y-%m-%d")
            high = temperature(nc.variables['high_tmpk_12z'][tstep, :, :],
                               'K').value('F')
            low = temperature(nc.variables['low_tmpk_12z'][tstep, :, :],
                              'K').value('F')
            pday = distance(nc.variables['p01d_12z'][tstep, :, :],
                            'MM').value('IN')
