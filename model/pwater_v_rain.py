...
def main():
    ...
    for row in mcursor:
        binval = int(row[1] / 4)
        if row[0].strftime("%Y%m%d") in obs:
            hits[binval] += 1
        total[binval] += 1
        pwater.append(row[1])
    ...
    bars = ax[0].bar(
        numpy.arange(0, 80, 4), percent, width=4.0, facecolor="lightblue"
    )
    ...
    for i in range(len(bars)):
        if total[i] > 0:
            ax[0].text(
                i * 4 + 2, percent[i] - 7, "%.0f" % (total[i],), ha="center"
            )
    ax[0].text(5, 80, "Bar label is \n# of days")
    ax[0].set_title(
        "Des Moines May Precipitable Water & Rainfall (2004-2012)\n"
