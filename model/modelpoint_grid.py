...
MOS = psycopg2.connect(
    database="mos", host="localhost", port=5555, user="nobody"
)
mcursor = MOS.cursor()
IEM = psycopg2.connect(
    database="iem", host="localhost", port=5555, user="nobody"
)
icursor = IEM.cursor()
...
for i, x in enumerate(xaxis[:-1]):
    icursor.execute(
        """SELECT sum(phour) from hourly_2016 where
...
for row in mcursor:
    ...
    ftime = row[1]
    precip = row[2]
    x = int((ftime - sts).total_seconds() / 10800.0) - 1
    y = int((runtime - msts).total_seconds() / 21600.0)
...
ax.set_title(
    ("GFS Grid Point Forecast for Ames\n" "3 Hour Total Precipitation [inch]")
)
