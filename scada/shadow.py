...
for azimuth in range(0, 360):
    ...
    cursor.execute(
        """
        SELECT count(*) from shadow where """
        + a
        + """
        and distance < %s
    """,
    ...
    cursor.execute(
        ...
        SELECT distinct aid from shadow where """
        + a
        + """
        and distance < %s)
    """,
    ...
    y2.append(cursor.fetchone()[0] / float(turbines) * 100.0)

(fig, ax) = plt.subplots(2, 1, sharex=True)
ax[0].set_title(
    ("Number of Turbine Shadows %.0f$^\circ$ at %.3f km")
    % (radius * 2, distance / 1000.0)
)
