...
def do(turbine_id):
    ...
    ax[0].set_title(
        "Turbine %s Bias from Wind Farm Average by Yaw Angle" % (turbine_id,)
    )
    for c in ["STABLE", "NEUTRAL", "UNSTABLE"]:
        df2 = df[df["tod"] == c]
        ax[0].step(df2["yaw"], df2["avg_wdiff"], lw=2, label=c)
    ax[0].set_xlim(0, 360)
    ...
    for c in ["STABLE", "NEUTRAL", "UNSTABLE"]:
        df2 = df[df["tod"] == c]
        ax[1].step(df2["yaw"], df2["avg_pdiff"], lw=2, label=c)
    ax[1].set_xlim(0, 360)
    ...
    writer = pd.ExcelWriter(
        "pdiff_%s.xlsx" % (turbine_id,), engine="xlsxwriter"
    )
