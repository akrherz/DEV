...
def main(argv):
    ...
    df["wcht"] = windchill(
        df["tmpf"].values * units("degF"), df["smph"].values * units("mph")
    )
    ...
    ax.set_title(
        ("NWS NDFD Grid Point Forecast for %s Airport") % (conf[school][4])
    )
    ax.grid(True)
    ...
    ax.axvspan(
        datetime.datetime(2019, 1, 29, 17),
        datetime.datetime(2019, 1, 31, 12),
        zorder=-1,
        color="tan",
    )
    ax.text(
        datetime.datetime(2019, 1, 30, 14),
        -55,
        "%s CLOSED!" % (school,),
        bbox=dict(color=conf[school][0]),
        ha="center",
        va="center",
        color=conf[school][1],
        fontsize=18,
