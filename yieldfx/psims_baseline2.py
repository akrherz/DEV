...
def make_netcdf(ncfn, valid, west, south):
    ...
    totaldays = (
        ...
        - valid.replace(year=1958, month=1, day=1)
    ).days + 1
...
def copy_iemre(nc, ncdate0, ncdate1, islice, jslice):
    ...
    with ncopen(rencfn) as renc:
        ...
        tidx0 = (ncdate0 - datetime.date(1958, 1, 1)).days
        tidx1 = (ncdate1 - datetime.date(1958, 1, 1)).days
...
def tile_extraction(nc, valid, west, south, fullmode):
    ...
    jslice = slice(j, j + 16)
    if fullmode:
        for year in range(1958, valid.year + 1):
            copy_iemre(
                nc,
...
def qc(nc):
    ...
    for i, time in enumerate(nc.variables["time"][:]):
        ts = datetime.date(1958, 1, 1) + datetime.timedelta(days=int(time))
...
def workflow(valid, ncfn, west, south, fullmode):
    ...
    basedir = "/mesonet/share/pickup/yieldfx/baseline1958"
