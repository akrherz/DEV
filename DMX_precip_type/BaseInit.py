...
class BaseInit(Forecaster):
    ...
    def __init__(self, srcName, dstName=None):
        ...
        self._WRtopoSites=["BYZ","TFX","MSO","PIH","SLC","FGZ","TWC","PSR","VEF","LKN","BOI","OTX","PDT",
                           "SEW","PQR","MFR","REV","EKA","STO","HNX","MTR","LOX","SGX"]
        self._SRtopoSites=["ABQ","EPZ","MAF","FFC","MRX","HUN","OHX"]
        self._ERtopoSites=["GSP","RNK","RLX","LWX","RLX","PBZ","CTP","BUF","BGM","ALY","BTV","GYX","CAR","BOX"]
        self._ARtopoSites=["AFG","AJK","AFC"]
        self._topoSites=self._CRtopoSites+self._WRtopoSites+self._SRtopoSites+self._ERtopoSites+self._ARtopoSites
        ## LogStream.logEvent("self._topoSites:", self._topoSites)

        self._GreatLakeSites=["DLH","MQT","APX","GRB","MKX","LOT","IWX","CLE","BUF","GRR","DTX"]
        self._prismSites = ["AKQ","ALY","BGM","BOX","BTV","BUF","CAE","CAR",
                            "CHS","CLE","CTP","GSP","GYX","ILM","ILN","LWX",
                            "MHX","OKX","PBZ","PHI","RAH","RLX","RNK"]

        # set up default options
        self.BI_optionsDict={'topoSite':False,
                             'prismSite':False,
                             'GreatLakeSite':False,
        ...
        if self._siteID in self._WRtopoSites or self._siteID in self._SRtopoSites:
    ...
    def BI_appendBoundaryLayers(self, blTemps, blRH, blWinds, 
                    p_SFC, stopo, gh_c, t_c, rh_c, wind_c, ctime, pdiff):
        ...
        if self.BI_optionsDict.get('useWetBulb',False):
           self.BI_BLE=self.Wetbulb(self.BI_BLT-273.15,self.BI_BLR,self.BI_BLP)
        self.BLcubeTime=ctime
        return

    # From: ../../methods/BI_adjustIsobaricCubes/BI_adjustIsobaricCubes.py
    def BI_adjustIsobaricCubes(self, BLP, BLT, BLD, BLH, blLevel, modelP_c,
                               t_c, modelTd_c, rh_c, gh_c, isobaric, intersect):

        #-----------------------------------------------------------------------
        #  Adjust the height of this isobaric level

        gh_c[isobaric][intersect] = (BLH[blLevel] +
                        self.BI_calcDeltaZ(BLT[blLevel], BLD[blLevel],
                            BLP[blLevel], modelP_c[isobaric] - BLP[blLevel])
                                         )[intersect]
        ...
        t_c[isobaric][intersect] = self.linear(
            BLH[blLevel-1], BLH[blLevel], BLT[blLevel-1], BLT[blLevel],
        ...
        if aboveBLcube.any():
            ...
            t_c[isobaric][aboveBLcube] = self.linear(
                BLH[blLevel], gh_c[isobaric + 1],
                BLT[blLevel], t_c[isobaric + 1], gh_c[isobaric])[aboveBLcube]
        ...
        modelTd_c[isobaric][intersect] = self.linear(
           self.BI_BLH[blLevel-1], self.BI_BLH[blLevel],
           self.BI_BLD[blLevel-1], self.BI_BLD[blLevel],
           gh_c[isobaric])[intersect]
        ...
        if aboveBLcube.any():
            ...
            modelTd_c[isobaric][aboveBLcube] = self.linear(
                BLH[blLevel], gh_c[isobaric + 1],
                BLD[blLevel], modelTd_c[isobaric + 1],
                gh_c[isobaric])[aboveBLcube]
            
        #-----------------------------------------------------------------------
        #  Adjust the relative humidity at these points

        rh_c[isobaric][intersect] = (100.0*self.esat(modelTd_c[isobaric])/
                                           self.esat(t_c[isobaric]))[intersect]
...
################################################################################
    ...
    def BI_baseSnowRatio(self, tGrid):
        ...
        tThresh = [-30.0, -21.0, -18.0, -15.0, -12.0, -10.0, -8.0, -5.0, -3.0,
                   2.0]

        #  Define polynomial coefficients for each temperature threshold
        a = [9.0, 21.0, 31.0, 35.0, 26.0, 15.0, 9.0, 5.0, 4.0]
        b = [0.4441, 3.1119, 2.8870, -0.6599, -5.2475, -4.5685, -1.9786,
             -0.7544, -0.3329]
        c = [0.0, 0.2964, -0.3714, -0.8109, -0.7183, 1.0578, 0.2372, 0.1709,
             0.0399]
        d = [0.0110, -0.0742, -0.0488, 0.0103, 0.2960, -0.1368, -0.0074,
             -0.0218, -0.0027]
...
################################################################################
    ...
    #  mask as needed
    #===========================================================================
    def BI_calcMarineScalar(self, grid, fillMasks=[], mask=None, maskValue=0.0,
                            minValue=0.0, filterValue=50.0):
    ...
    #  mask as needed
    #===========================================================================
    def BI_calcMarineVector(self, gridMag, gridDir, fillMasks=[], mask=None,
                            maskValue=0.0, minValue=0.0, filterValue=50.0):

        #  Convert magnitude from m to ft - filtering out everything above
        #  filter value
        gridMag = where(greater(gridMag, filterValue), minValue,
                        gridMag)
    ...
    #===========================================================================
    def BI_calcMaxT(self, T, MaxT, mtime, modelSfcT=None, modelMaxT=None, offset=0):
...
##                             thetaToFind, 0.0)
    ...
    def BI_calcMosWind(self, swind, stopo, topo, applyTopo=0, fillMasks=[]):
        ...
        if applyTopo:
            ...
            #  Apply lapse rate correction where real topo is above model topo
            wmag = where(greater(topoDiff, 0.0),
                         wmag + (spd_lapse_rate * topoDiff), wmag)
...
##        wdir = self.BI_smoothpm(wdir, 1)
    ...
    #    a list of Numeric/numpy masks through the fillMasks argument.
    #===========================================================================
    def BI_calcMosWindGust(self, gust, wmag, stopo, topo, applyTopo=0,
                           fillMasks=[]):
        ...
        if applyTopo:
            ...
            #  Apply lapse rate correction where real topo is above model topo
            gust = where(greater(topoDiff, 0.0),
                         gust + (spd_lapse_rate * topoDiff), gust)
    ...
    def BI_calcPredHgtRH(self, gh_c, rh_c, topo):
        ...
        cloudRH = [98.0, 96.0, 94.0, 92.0, 90.0,    #  1000-900 mb (25 mb)
                   88.0, 85.0, 83.0, 80.0, 78.0,    #  875-775 mb  (25 mb)
                   75.0, 73.0, 70.0, 68.0, 65.0,    #  750-650 mb  (25 mb)
                   63.0, 60.0, 58.0, 55.0, 53.0,    #  625-525 mb  (25 mb)
                   50.0, 45.0, 40.0, 35.0, 30.0     #  500-300 mb  (50 mb)
                  ]
        ...
        for index in xrange(gh_c.shape[0]):
            ...
            cloudBaseHgt = where(
                logical_and(equal(cloudBaseHgt, MaxCloudBase),
                            greater_equal(rh, cloudRH[index])),
                            height/100, cloudBaseHgt)
...
################################################################################
    ...
    def BI_calcRH(self, t, dpt, ctime):
        ...
        if not cube and self._rh_FHAG2 is not None and ctime == self.BLcubeTime:
        ...
        if not cube:
            ...
            #  Surface RH
            self._rh_FHAG2 = 100.0 * (self.VAPR(dpt - 273.15) /
                                      self.VAPR(t - 273.15) )
            ...
            #  RH cube
            self._rh_c = 100.0 * (self.VAPR(dpt - 273.15) /
                                  self.VAPR(t - 273.15) )
    ...
    #    a list of Numeric/numpy masks through the fillMasks argument.
    #===========================================================================
    def BI_calcSky(self, blTemps, blRH, blWinds, p_SFC, stopo, topo, gh_c,
                   t_c, rh_c, wind_c, ctime, MSLP=0, fillMasks=[]):

        #  Setup the boundary layer cube for this time - if needed
        self.BI_setupBLCube(blTemps, blRH, blWinds, p_SFC, stopo, topo, gh_c,
                            t_c, rh_c, wind_c, ctime, MSLP)
...
##        LogStream.logEvent("Now there are %d levels for sky." % (f.shape[0]))
        ...
        if len(f) >= 5:
            ...
            LogStream.logEvent("WARNING: Sky data is missing some levels - " +
    ...
    #    a list of Numeric/numpy masks through the fillMasks argument.
    #===========================================================================
    def BI_calcSnowRatio(self, gh_c, t_c, rh_c, pvv_c, gh_MB925, gh_MB800,
                         gh_MB850, gh_MB750, gh_MB700, gh_MB650, gh_MB600,
                         gh_MB550, cutBelow=4, thickness="925-700",
                         fillMasks=[]):
        ...
        for i in range(len(gh_c) - 1):
            ...
            pvvAvg[i] = where(less(avgRH, 80.0),
        ...
        if thickness == "850-700":
            thicknessSnowRatio = \
                20.0 - pow(((gh_MB700 - gh_MB850) - 1437.0) / 29.0 , 2)
        elif thickness == "925-700":
            thicknessSnowRatio = \
                20.0 - pow(((gh_MB700 - gh_MB925) - 2063.0) / 41.0 , 2)
        elif thickness == "850-650":
            thicknessSnowRatio = \
                20.0 - pow(((gh_MB650 - gh_MB850) - 1986.0) / 39.0 , 2)
        elif thickness == "800-600":
            thicknessSnowRatio = \
                20.0 - pow(((gh_MB600 - gh_MB800) - 2130.0) / 42.0 , 2)
        else:       # "750-550"
            thicknessSnowRatio = \
                20.0 - pow(((gh_MB550 - gh_MB750) - 2296.0) / 45.0 , 2)

        #  Ensure thickness snow ratio is not less than zero
        thicknessSnowRatio = where(less(thicknessSnowRatio, 0.0),
                                   0.0, thicknessSnowRatio)
        ...
        totalSnowRatio = where(less_equal(pvvSum, 100.0),
                               ((totalSnowRatio * 0.01 * pvvSum) +
                                (thicknessSnowRatio * (1.0 - pvvSum * 0.01))),
                               totalSnowRatio)
        totalSnowRatio = where(less(pvvSum, 1.0), thicknessSnowRatio,
                                                  totalSnowRatio)
    ...
    def BI_calcT(self, blTemps, blRH, blWinds, p_SFC, t_SFC, stopo, topo,
                 gh_c, t_c, rh_c, wind_c, ctime, MSLP=0, fillMasks=[]):
...
################################################################################
    ...
    #===========================================================================
    def BI_calcTd(self, blTemps, blRH, blWinds, p_SFC, T, stopo, topo, gh_c,
                  t_c, rh_c, wind_c, ctime, MSLP=0, fillMasks=[]):
    ...
    #    a list of Numeric/numpy masks through the fillMasks argument.
    #===========================================================================
    def BI_calcTransWind(self, blTemps, blRH, blWinds, p_SFC, MixHgt, stopo,
                         topo, gh_c, t_c, rh_c, wind_c, ctime, MSLP=0,
                         fillMasks=[]):
...
################################################################################
    ...
    #===========================================================================
    def BI_calcWind(self, blTemps, blRH, blWinds, p_SFC, stopo, topo, gh_c,
                    t_c, rh_c, wind_c, ctime, MSLP=0, fillMasks=[]):
        ...
        if self.BI_optionsDict['topoSite']:
            ...
            for i in range(1, BLH.shape[0]):
                ...
                between = logical_and(greater_equal(topo, BLH[i-1]),
                                      less(topo, BLH[i]))

                #  Interpolate model winds to real topography
                mval = self.linear(BLH[i], BLH[i-1], BLMAG[i], BLMAG[i-1], topo)
                dval = self.BI_dirlinear(BLH[i], BLH[i-1], BLDIR[i], BLDIR[i-1],
                                         topo)
            ...
            smag = where(greater(smag, BLMAG[0]),
                         BLMAG[0] + ((smag - BLMAG[0]) / 2.0), smag)
...
##        wdir = self.BI_smoothpm(wdir, 1)
    ...
    def BI_categorizeVsby(self, vsby, convert=0):
        ...
        #  6 SM
        vsby = where(logical_and(greater(vsby, 5.45), less(vsby, 6.5)),
                     6.0, vsby )
        #  5 SM
        vsby = where(logical_and(greater(vsby, 4.45), less(vsby, 5.46)),
                     5.0, vsby)
        #  3 SM
        vsby = where(logical_and(greater(vsby, 2.45), less(vsby, 4.46)),
                     3.0, vsby)
        #  2 SM
        vsby = where(logical_and(greater(vsby, 1.45), less(vsby, 2.46)),
                     2.0, vsby)
        #  1 SM
        vsby = where(logical_and(greater(vsby, 0.85), less(vsby, 1.46)),
                     1.0, vsby)
        #  3/4 SM
        vsby = where(logical_and(greater(vsby, 0.65), less(vsby, 0.86)),
                     0.75, vsby)
        #  1/2 SM
        vsby = where(logical_and(greater(vsby, 0.30), less(vsby, 0.66)),
                     0.50, vsby )
        #  1/4 SM
    ...
    def BI_checkT(self, T):
        ...
        """
        if T is not None and T.min() >= self.BI_optionsDict['noSnowThresholdT']:
            return False
        return True
    ...
    #    a list of Numeric/numpy masks through the fillMasks argument.
    #===========================================================================
    def BI_computeSkyLayer(self, pressureLevel, rhLayer, ghLayer,
                           startPressureLevel, topo, fillMasks=[]):
        skyLayer = 0
        pressureDiff = pressureLevel - startPressureLevel
        skyLayer = where(equal(pressureDiff, 0),
                        where(rhLayer < 64, 0, rhLayer * 1.13 - 71.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 2),
                        where(rhLayer < 51, 0, rhLayer * 1.33 - 66.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 4),
                        where(rhLayer < 59, 0, rhLayer * 1.65 - 95.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 6),
                        where(rhLayer < 46, 0, rhLayer * 1.63 - 73.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 10),
                        where(rhLayer < 35, 0, rhLayer * 1.79 - 61.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 14),
                        where(rhLayer < 20, 0, rhLayer * 1.20 - 24.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 18),
                        where(rhLayer < 10, 0, rhLayer * 0.86 - 7.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 20),
                        where(rhLayer < 0, 0, rhLayer * 0.55 + 3.0), skyLayer)
        skyLayer = where(equal(pressureDiff, 22),
                        where(rhLayer < 0, 0, rhLayer * 0.33 + 5.0), skyLayer)
...
################################################################################
    ...
    def BI_EnhanceTopoPoP(self, PoP, topo):
        ...
        multiplier = where(greater(topo, 10.0),
                           0.7213 * log(topo) - 4.4141, 1.0)

        PoP = where(logical_and(greater(topo, 1818), less(topo, 3636)),
                    PoP * multiplier, PoP)
    ...
    def BI_fillMissingClimo(self,grid,zeroMask=None):
        ...
        newGrid = where(logical_and(equal(zeroMask,0.0),greater_equal(smoothMask,0.5)),smoothQPFC,grid)
        ...
        for i in range(2,200,1):
            ...
            newGrid = where(logical_and(equal(newGrid,0.0),greater_equal(smoothMask,0.2)),smoothQPFC,newGrid)
...
##        mygrid = PredHgt
    ...
    def BI_GeneralPoPBL(self, t_FHAG2,   t_BL030,   t_BL3060,   t_BL6090,   t_BL90120,   t_BL120150,
                   rh_FHAG2,  rh_BL030,  rh_BL3060,  rh_BL6090,  rh_BL90120,  rh_BL120150,
                wind_FHAG10,wind_BL030,wind_BL3060,wind_BL6090,wind_BL90120,wind_BL120150,
                p_SFC, QPF, stopo, topo, gh_c, t_c, rh_c, wind_c, ctime, stime, gridLengthFactor=1):
        ...
        self.BI_setupBLCube([ t_FHAG2,  t_BL030,  t_BL3060,  t_BL6090,  t_BL90120,  t_BL120150],
                            [rh_FHAG2, rh_BL030, rh_BL3060, rh_BL6090, rh_BL90120, rh_BL120150],
                            [wind_FHAG10, wind_BL030, wind_BL3060, wind_BL6090, wind_BL90120, wind_BL120150],
    ...
    def BI_getClimoQPF(self, ctime):
        ...
        (ctime1, ctime2) = ctime
        (cyea, cmon, cday, chou, cmin, csec, cwda, cyda, cdst) = \
               time.gmtime(ctime1)
        if (cday < 16):
            pyea = cyea
            pmon = cmon - 1
    ...
    def BI_getFhr(self):
        ...
        return (validTime.getTime()/1000 - self.sourceBaseTime())/3600.
    ...
    def BI_getMosTopoDiff(self, stopo, topo):
        ...
        fileName = "/awips2/edex/data/utility/edex_static/site/%s/config/gfe/GFS_topo.dat"%(self._siteID)
        ...
        try:
            ...
            LogStream.logEvent("  %s does not exist! " % (fileName) +
    ...
    def BI_getPoPfromRadar(self,radar,ctime,Low=-15):
        ...
        PoP = where(less(radar, dbz1-fuzzVal), Low, PoP) #all values min pop
        PoP = where(logical_and(greater_equal(radar, dbz1-fuzzVal),less_equal(radar, dbz1)),interPoP1, PoP) #interpolated values in between
        PoP = where(logical_and(greater_equal(radar, dbz1),less(radar, dbz2)),interPoP2, PoP) #interpolated values in between
    ...
    def BI_getQPFPct(self,QPF,ctime):
        ...
        crange=cmax - cmin
        climoSmooth=self.BI_smoothpm(qpfclim,
                                     self.BI_optionsDict['ClimoQPFSmooth'])
    ...
    def BI_getWetbulbCross(self, gh_c, t_c, rh_c, topo):
        ...
        while ((index>=0)and(numLeft>0)):
            ...
            cross=logical_and(less(height,-5000),greater_equal(wet,threshk))
            height=where(cross,val,height)
            #numfound=add.reduce(add.reduce(cross))
            #self.logtime("   crossings at level %d (%d): %d"%(index,self.pres[index],numfound))
            #
    ...
    def BI_LakeSnowParameter(self, T850, rh850, rh700, wind850, wind700, ctime):
    ...
    def BI_LESPoP(self, PoP,LSP,wind925,wind850,ctime):
        ...
        if lesdir is not None:
           ...
           for lesArea,dirs in lesdir.iteritems():
               ...
               if primaryArea in self._editAreas and secondaryArea in self._editAreas:
                  found=1
                  ...
                  if (dir1 > dir2): # for directional bounds surrouding due north
                      if (avgDir >=dir1) or (avgDir<=dir2):
                          #run smart tool on individual edit area to determine PoP
                          PoP = where(secondaryAreamask,self.BI_lsppop(LSP,PoP,.75,ctime),PoP)
                          PoP = where(primaryAreamask,self.BI_lsppop(LSP,PoP,1.0,ctime),PoP)
                  else:
    ...
    def BI_lsppop(self,LSP,PoP,multiplier,ctime):
        ...
        lespop = (40.259*LSP)+0.18 # just a simple linear regression to fit PoP to LSP
        lespop = where(less_equal(LSP,0.45),10,lespop) # LSP <= 0.45 -> PoP = 10
        lespop = where(less_equal(LSP,0.25),0,lespop) # LSP <= 0.25 -> PoP = 0
    ...
    def BI_setupBLCube(self, blTemps, blRH, blWinds, p_SFC, stopo, topo, gh_c,
                       t_c, rh_c, wind_c, ctime, MSLP=0, masterPdiff=None):
        ...
        #
        if len(blTemps) != len(blRH) or len(blTemps) != len(blWinds) or \
           len(blTemps) == 0:
        ...
        if not self.BI_optionsDict.get('interLeaveBL',False):
            self.BI_appendBoundaryLayers(blTemps, blRH, blWinds,
                       p_SFC, stopo, gh_c,
                       t_c, rh_c, wind_c, ctime, pdiff)
            return
        #
        #  split pressure level wind cube into magnitude and direction
        #
        ...
        if self.BI_optionsDict['useWetBulb']:
           self.BI_BLE = self.Wetbulb(self.BI_BLT - 273.15, self.BI_BLR,
                                         self.BI_BLP)
        #
...
#        self.pSFCmbTopo = pSfc
    ...
    def BI_setupClimoQPF(self):
        ...
        prismdb = self.getDb("%s_GRID_D2D_PRISMClimo_19700101_0000" %
                             (self._siteID))
...
################################################################################
    ...
    def BI_ThickSnowRatio(self, T, MaxTAloft, gh_MB1000, gh_MB500):
        ...
        sRatio = where(logical_and(less_equal(thick,5490),greater(thick,5460)),2.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5460),greater(thick,5400)),5.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5400),greater(thick,5370)),8.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5370),greater(thick,5340)),10.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5340),greater(thick,5310)),12.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5310),greater(thick,5280)),15.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5280),greater(thick,5220)),17.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5220),greater(thick,5160)),20.0,sRatio)
        sRatio = where(logical_and(less_equal(thick,5160),greater(thick,20)),25.0,sRatio)
        #
        if MaxTAloft is not None:
           sRatio = where(logical_or(greater_equal(T,35.6),greater_equal(MaxTAloft,0.0)),0,sRatio)
        else:
    ...
    def BI_weightSnowLevel(self, gh_c, rh_c, topo):
        ...
        rhavg = where(count, rhavg / (count + .001), 0)
