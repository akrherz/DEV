...
class NAM12(BaseInit.BaseInit):
    ...
    def rhWaterToIce(self, rhWaterGrid, temp):
        ...
        vaporPressureWRTwater = math.exp(53.67957-(6743.769/tempInK)-4.8451*naturalLogTemp)
        satVaporPressureWRTice = math.exp(23.33086-(6111.72784/tempInK)+0.15215*naturalLogTemp)
        rhIceGrid = rhWaterGrid * vaporPressureWRTwater/satVaporPressureWRTice
    ...
    def calcHaines(self, t_c, rh_c):
        return self.BI_hainesIndex(self.BI_optionsDict["HainesLevel"], t_c, rh_c)
    ...
    def calcMaxTAloft(self, t_c, gh_c, topo):
        ...
        for i in xrange(len(t_c) - 1):   #for each pressure level
            ...
            Mask = (self.maskAboveGround & startMaxTAloftMask & (temp > MaxTAloft))
    ...
    def calcMixHgt(self, t_FHAG2,  t_BL030,  t_BL3060,  t_BL6090,  t_BL90120,  t_BL120150,
    ...
    def calcCloudBasePrimary(self,topo, rh_c, gh_c):
        ...
        for i in xrange(rh_c.shape[0]):
            ...
            CloudBasePrimary = where(logical_and(equal(CloudBasePrimary, MaxCloudBase),greater_equal(rh, self.cloudRH[i])), height/100, CloudBasePrimary)
    ...
    def calcCloudBaseCCL(self,topo, T, Td, t_c, rh_c, gh_c, p_SFC, 
                             t_BL030, t_BL3060,rh_BL030, rh_BL3060):
        MaxCloudBase=250
...
#        
            readyToSet = logical_and(newWsLower,logical_and(maskAboveGround, less(lastCCL, topo + 1.0)))
...
#
    ...
    #Diurnal CU.  Adopted from GSP's CloudBase_FM_CCL_LCL tool
    def calcCloudBaseLCL(self,topo, T, Td, t_BL030, t_BL3060,rh_BL030, rh_BL3060):
        t_SFC=self.FtoK(T)
        td_SFC=self.FtoK(Td)
        
        dpt_BL030=self.FtoK(self.BI_dewFromTandRH(self.KtoF(t_BL030),rh_BL030))
        dpt_BL3060=self.FtoK(self.BI_dewFromTandRH(self.KtoF(t_BL3060),rh_BL3060))
...
#       calculate a boundary layer average dewpt (in Kelvins)
    ...
    def calcProbIcePresent(self, t_c, rh_c, gh_c, topo, MaxTAloft):
        ...
        for i in xrange(len(gh_c) - 1):
            ...
            if i > 1:
                ...
                currentLevelGHrhDry = (currentLevelGHrhDry | previousLevelGHrhDry)
                #currentGHrhDry = logical_or(less(rh_c[i], 75), less(rh_c[i-1], 75))
            if i > 2:
                currentLevelGHrhDry = (currentLevelGHrhDry | previousLevelGHrhDry | (rh_c[i-2] < 75))
                #currentGHrhDry = logical_or(logical_or(less(rh_c[i], 75), less(rh_c[i-1], 75)), less(rh_c[i-2], 75))
            Mask = (currentLevelGHrhDry & bottomGHrhDry & ((currentLevelGH - bottomGH) > 1500) & (bottomGH < allowableMaxGH))
            allowableMaxGH[Mask] = bottomGH[Mask]

            bottomGH[~currentLevelGHrhDry] = currentLevelGH[~currentLevelGHrhDry]
            Mask = bottomGH > 0
        ...
        for requestedTemp in temperatureListAboveZero:
            ...
            for i in xrange(len(t_c) - 1):
                ...
                if not array_equal(prevRH, self.empty()):
                    ...
                    MaskStayingAboveGround = (maskAboveGround & prevMaskAboveGround)
                    MaskRHHigher = rhRequestedTemp > rhGrid[requestedTemp]
                    Mask = (MaskHeightBelowMax & MaskStayingAboveGround & (prevTemp < requestedTemp * -1) & (temp > requestedTemp * -1) & MaskRHHigher)
                    rhGrid[requestedTemp][Mask] = rhRequestedTemp[Mask]
                    Mask = (MaskHeightBelowMax & MaskStayingAboveGround & (prevTemp > requestedTemp * -1) & (temp < requestedTemp * -1) & MaskRHHigher)
                    rhGrid[requestedTemp][Mask] = rhRequestedTemp[Mask]

                else:
                    Mask = (MaskHeightBelowMax & maskAboveGround & equal(temp, requestedTemp * -1) & (rh_c[i] > rhGrid[requestedTemp]))
                    rhGrid[requestedTemp][Mask] = rh_c[i][Mask]

                prevTemp = temp
                prevRH = rh_c[i]
            #Change rhGrid from w.r.t. water to w.r.t. ice
            rhGrid[requestedTemp] = self.rhWaterToIce(rhGrid[requestedTemp],requestedTemp*-1)
            #Smooth the rhGrid
            rhGrid[requestedTemp] = self.BI_smoothpm(rhGrid[requestedTemp], 3)
            #Create the probability of Ice for the requestedTemp
            probIceForTemp = (rhGrid[requestedTemp]/100 * self.probIceForTemp(requestedTemp)/100) * 100
    ...
    def calcProbRefreezeSleet(self, t_c, gh_c, topo, MaxTAloft):
        ...
        for i in xrange(len(t_c) - 1):   #for each pressure level
            ...
            if not array_equal(prevHeight, self.empty()):
                ...
                Mask = (maskAboveGround & (prevTemp < 0) & (temp > 0) & ZeroCHgtBelowMaxTAloft)
        ...
        for i in xrange(len(gh_c) - 1):
            maskAboveGround = gh_c[i] > topo
            temp = t_c[i] - 273.15 # Convert to C
            Mask = (maskAboveGround & (gh_c[i] < HeightFreezingLevel) & (temp < MinTAloft))
        ...
        ProbRefreezeSleet[Mask] = (2.3036 * MinTAloft * MinTAloft + 5.979 * MinTAloft + 0.8295)[Mask]
    ...
    def calcSky(self, rh_c, gh_c, topo, PoP):
        ...
        if self.BI_optionsDict['topoSite']:
            for i in xrange(rh_c.shape[0]):
                ...
                startPressureLevel = where(logical_and(equal(startPressureLevel,0), greater_equal(ghLayer - 50, topo)), i, startPressureLevel)
                skylayer = where(greater_equal(ghLayer - 50, topo), self.BI_computeSkyLayer(i, rhLayer, ghLayer, startPressureLevel, topo), 0)
                sky = sky + skylayer
        ...
        # coding for non-mountain offices
            ...
            for i in xrange(rh_c.shape[0]):
                ...
                skylayer = 0
                if i == 2:
                    skylayer = where(less(rhLayer, 64), 0, rhLayer * 1.13 - 71.0)   #950mb
                    count = where(greater(rhLayer, 90), count - 1, count)
                    count = where(greater(rhLayer, 95), count - 2, count)
                elif i == 4:
                    skylayer = where(less(rhLayer, 51), 0, rhLayer * 1.33 - 66.0)   #900mb
                    count = where(greater(rhLayer, 90), count - 1, count)
                    count = where(greater(rhLayer, 95), count - 2, count)
                elif i == 6:
                    skylayer = where(less(rhLayer, 59), 0, rhLayer * 1.65 - 95.0)   #850mb
                    count = where(greater(rhLayer, 90), count - 1, count)
                    count = where(greater(rhLayer, 95), count - 2, count)
                elif i == 8:
                    skylayer = where(less(rhLayer, 46), 0, rhLayer * 1.63 - 73.0)   #800mb
                    count = where(greater(rhLayer, 90), count - 1, count)
                    count = where(greater(rhLayer, 95), count - 2, count)
                elif i == 12:
                    skylayer = where(less(rhLayer, 35), 0, rhLayer * 1.79 - 61.0)   #700mb
                elif i == 16:
                    skylayer = where(less(rhLayer, 20), 0, rhLayer * 1.20 - 24.0)   #600mb
                elif i == 20:
                    skylayer = where(less(rhLayer, 10), 0, rhLayer * 0.86 - 7.0)    #500mb
                elif i == 21:
                    skylayer = where(less(rhLayer, 0), 0, rhLayer * 0.55 + 3.0)     #450mb
                elif i == 22:
                    skylayer = where(less(rhLayer, 0), 0, rhLayer * 0.33 + 5.0)     #350mb
        ...
        sky = where(greater_equal(PoP, 15), where(less(sky, skycheck), skycheck, sky), sky)
        sky = self.BI_smoothpm(sky, 3)
    ...
    def calcSnowAmt(self, QPF, T, SnowRatio):
        ...
        SnowAmt = where(greater(T, 32.0), pow(36.0 - T,2)/16.0 * SnowAmt , SnowAmt)
    ...
    def calcSnowRatio(self, T, gh_c, t_c, rh_c, pvv_c, gh_MB925, gh_MB800, gh_MB850, gh_MB750, gh_MB700, gh_MB650, gh_MB600, gh_MB550):

        if not self.BI_checkT(T):
           return where(T, 0, 0)

        #Cobb method
        return self.BI_calcSnowRatio(gh_c, t_c, rh_c, pvv_c, gh_MB925, gh_MB800,
    ...
    def calcT(self, t_FHAG2, t_BL030, t_BL3060, t_BL6090, t_BL90120, t_BL120150,
              rh_FHAG2, rh_BL030, rh_BL3060, rh_BL6090, rh_BL90120, rh_BL120150,
              wind_FHAG10, wind_BL030, wind_BL3060, wind_BL6090, wind_BL90120,
              wind_BL120150, p_SFC, stopo, topo, gh_c, t_c, rh_c, wind_c,
              ctime):

        #  Construct lists of boundary layer data
        blTemps = [t_FHAG2, t_BL030, t_BL3060, t_BL6090, t_BL90120, t_BL120150]
        blRH = [rh_FHAG2, rh_BL030, rh_BL3060, rh_BL6090, rh_BL90120,
                rh_BL120150]
        blWinds = [wind_FHAG10, wind_BL030, wind_BL3060, wind_BL6090,
                   wind_BL90120, wind_BL120150]

        #  Compute the temperature using the available boundary layer data
        return self.BI_calcT(blTemps, blRH, blWinds, p_SFC, t_FHAG2,
                             stopo, topo, gh_c, t_c, rh_c, wind_c, ctime)
    ...
    def calcTd(self, t_FHAG2, t_BL030, t_BL3060, t_BL6090, t_BL90120,
        ...
        return self.BI_calcTd(blTemps, blRH, blWinds, p_SFC, T, stopo, topo,
    ...
    def calcTransWind(self, t_FHAG2, t_BL030, t_BL3060, t_BL6090, t_BL90120,
                      ...
                      p_SFC, MixHgt, stopo, topo, gh_c, t_c, rh_c, wind_c,
                      ctime):
        ...
        return self.BI_calcTransWind(blTemps, blRH, blWinds, p_SFC, MixHgt,
                                     stopo, topo, gh_c, t_c, rh_c, wind_c,
                                     ctime)
    ...
    def calcWind(self, t_FHAG2, t_BL030, t_BL3060, t_BL6090, t_BL90120,
        ...
        if self.BI_optionsDict['topoSite']:
            ...
            ew= (6.1094* exp(17.625*(bl_temp[0]-273.15)/(243.04+(bl_temp[0]-273.15))))*(bl_relh[0]/100.0)
            ew= ew*1.00071*exp(.0000045*bl_pres[0]/100.0)
            bl_tvrt.append((bl_pres[0]/100.0)/((bl_pres[0]/100.0)+(ew*(eps-1.0)))*bl_temp[0])

            # Grab the boundary layer data.

            for bl in [wind_BL030,wind_BL3060,wind_BL6090,wind_BL90120,wind_BL120150] :
                bl_wdir.append(bl[1])
                bl_wspd.append(bl[0])
            ...
            for bl in [rh_BL030,rh_BL3060,rh_BL6090,rh_BL90120,rh_BL120150] :
                bl_relh.append(bl)

            for lev in range(1,num_bl+1) :
                ew= (6.1094* exp(17.625*(bl_temp[lev]-273.15)/(243.04+(bl_temp[lev]-273.15))))*(bl_relh[lev]/100.0)
                ew= ew*1.00071*exp(.0000045*bl_pres[lev]/100.0)
                bl_tvrt.append((bl_pres[lev]/100.0)/((bl_pres[lev]/100.0)+(ew*(eps-1.0)))*bl_temp[lev])
            ...
            for lev in range(1,num_bl+1,1):
                bl_hght.append(bl_hght[lev-1]+R_d/g*(bl_tvrt[lev-1]+bl_tvrt[lev])/2.0*log(bl_pres[lev-1]/bl_pres[lev]))
            ...
            for lev in range(1,num_bl+1):
                between= logical_and(greater_equal(topo_real,bl_hght[lev-1]),less(topo_real,bl_hght[lev]))
                spd= where(equal(1,between),self.linear(bl_hght[lev-1],bl_hght[lev],bl_wspd[lev-1],bl_wspd[lev],topo_real),spd)
                dir= where(equal(1,between),self.BI_dirlinear(bl_hght[lev-1],bl_hght[lev],bl_wdir[lev-1],bl_wdir[lev],topo_real),dir)
            ...
            spd= where(greater(spd,bl_wspd[0]),bl_wspd[0]+((spd-bl_wspd[0])/2.0),spd)
