...
#   calculate dewpoint for the 30-60mb layer
    	...
    	tdBL3060 = (243.5 * log(eBL3060 / 6.112))/ (17.67 - log(eBL3060 / 6.112))
...
#   calculate first guess Tw
    	tW3060 = ((tK3060 * f * pSFC) + (tdK3060 * s3060)) / ((f * pSFC) + s3060)
#    Solve for tW iteratively by attempting an energy balance
#    Ten steps should be sufficient
...
#    	return MinTwAloft
	...
	#ColdLayerDepth = where(logical_and(greater(gh_c[i], topo), logical_and(less(gh_c[i], HeightMaxTAloft), less(t_c[i], 0.0))), gh_c[i], ColdLayerDepth)
        	previousHeight = where(logical_and(greater(gh_c[i], 0.0),
                                 logical_and(less(gh_c[i], HeightMaxTAloft), less(t_c[i], 273.15))), gh_c[i - 1], 0)
        	height = where(logical_and(greater(gh_c[i], 0.0),
                                 logical_and(less(gh_c[i], HeightMaxTAloft), less(t_c[i], 273.15))), gh_c[i], 0)
        ...
        if QPF is None:
           newPoP = 0
            ...
            for yy in QPF:
                xval = 0
                for xspot in yy:
                    newPoP[yval][xval] = self._makeSquarePoP(QPF,gp,[yval,xval],thresh)
                    xval += 1
                yval += 1        
