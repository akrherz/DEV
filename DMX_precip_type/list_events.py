...
def grouper():
    ...
    rows = []
    for (tmpf, dwpf), gdf in df[((df['vaid'].dt.year >= decade) &
                                 (df['vaid'].dt.year < (decade + 10)))].groupby(['tmpf', 'dwpf']):
        ...
        rows.append(dict(tmpf=tmpf, dwpf=dwpf, snowcount=snowcount,
                         raincount=raincount))
...
def plot4():
    ...
    labels = {'i_wetbulb': 'Wet Bulb', 'dwpf': 'Dew Point',
              'tmpf': 'Air Temperature'}
    for state in df['state'].unique():
        gdf = df[df['state'] == state].groupby('i_wetbulb').sum()
        total = gdf['snow'] + gdf['zr'] + gdf['rain']
        ax.plot(gdf.index.values, gdf['snow'] / total * 100., label=state)
    ax.grid(True)
    ax.legend(loc=6)
    ...
    ax.set_title(("For Precipitating Events\n"
...
def plot3():
    ...
    for i, label in enumerate(['i_wetbulb', 'tmpf', 'dwpf']):
        ...
        ax.set_xticks(range(20, 41, 2))
        ax.set_xlim(20, 40)
        ax.text(1.01, 0.5, "%s" % (labels[label], ), transform=ax.transAxes,
...
def plot2():
    ...
    for varname in ['tmpf', 'dwpf', 'i_wetbulb']:
        ...
        ax.plot(gdf.index.values,
                gdf['snowcount'] / (gdf['raincount'] + gdf['snowcount']) * 100.,
                label=varname)
    ax.legend()
    ax.grid(True)
    ax.set_xlim(20, 40)
...
def workflow(pgconn, sid):
    ...
    df['snow'] = df['presentwx'].apply(is_in, args=(['-SN', 'SN', '+SN',
                                                     ...
                                                     '-TSSN', 'TSSN',
                                                     '+TSSN'], ))
    df['rain'] = df['presentwx'].apply(is_in, args=(['-RA', 'RA', '+RA',
                                                     '-RASN',
                                                     '-TSRA', 'TSRA',
                                                     '+TSRA'],))
...
def main():
    ...
    nt = NetworkTable(("IA_ASOS", 'MN_ASOS', 'WI_ASOS', 'MI_ASOS', 'OH_ASOS',
                       'IN_ASOS', 'KY_ASOS', 'IL_ASOS', 'MO_ASOS', 'KS_ASOS',
                       'NE_ASOS', 'ND_ASOS', 'SD_ASOS'))
    dfs = []
    for sid in tqdm.tqdm(nt.sts):
        if (nt.sts[sid]['archive_begin'] and
                nt.sts[sid]['archive_begin'].year < 1980):
