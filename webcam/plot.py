...
def main():
    ...
    bars = ax.bar(
        [0.2, 1.2, 2.2],
        [rise_hits / rise_cases * 2.0, set_hits / set_cases * 2.0, 0.246 * 2],
        width=0.6,
        zorder=3,
    )
    ...
    ax.set_xticklabels(
        [
            "Red Sky @ Morning\n%.0f/%.0f %.0f"
            % (rise_hits, rise_cases, sunrises),
            "Red Sky @ Night\n%.0f/%.0f %.0f" % (set_hits, set_cases, sunsets),
            "All Days",
        ]
    )
    ...
    txt = ax.text(
        ...
        1.5,
        (
            "Does a Red Sky predict Precipitation?\n"
