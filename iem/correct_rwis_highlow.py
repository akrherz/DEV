...
NETWORK = NetworkTable([
    "IL_RWIS", "IA_RWIS", "AK_RWIS", "WV_RWIS", "KY_RWIS",
    "NE_RWIS", "ND_RWIS", "WY_RWIS", "ME_RWIS", "NH_RWIS", "GA_RWIS",
    "MO_RWIS", "KS_RWIS", "IN_RWIS", "MD_RWIS", "VA_RWIS", "VT_RWIS",
    "WI_RWIS", "SD_RWIS", "MN_RWIS", "CO_RWIS", "NV_RWIS", "OH_RWIS",
    "DE_RWIS", "MI_RWIS", "MA_RWIS"])


def main():
    """Do Some workflow"""
    iemaccess = psycopg2.connect(database='iem', host="localhost", port=5555,
                                 user='mesonet')
    rwis = psycopg2.connect(database='rwis', host="localhost", port=5555,
                            user='nobody')
    for sid in NETWORK.sts:
        ...
        for date, row in df[idx].iterrows():
            ...
            """, (row['max'], row['min'], NETWORK.sts[sid]['iemid'],
                  date))
            updated += 1
        print(("Updated %s rows for %s between %s and %s"
