...
def main():
    ...
    ax[0].add_patch(rect)
    ax2.text(
        0.6, 0.8, "NW Winds push\nwater to gauge", transform=ax2.transAxes
    )

    ax[0].plot(valid["HPIRGZ"], values["HPIRGZ"], zorder=2)

    dt = (valid["USIRGZ"][-1] - valid["USIRGZ"][0]).days * 86400.0 + (
        valid["USIRGZ"][-1] - valid["USIRGZ"][0]
    ).seconds
    ...
    ax[0].xaxis.set_major_locator(mdates.DayLocator(interval=1, tz=central))
    ...
    ax3.text(
        0.15,
        0.8,
        "SE Winds push\nwater away from gauge",
        transform=ax3.transAxes,
    )

    ax[1].plot(valid2["HPIRGZ"], values2["HPIRGZ"], zorder=2)

    dt = (valid2["USIRGZ"][-1] - valid2["USIRGZ"][0]).days * 86400.0 + (
        valid2["USIRGZ"][-1] - valid2["USIRGZ"][0]
    ).seconds
    ...
    ax[1].xaxis.set_major_locator(mdates.DayLocator(interval=1, tz=central))
