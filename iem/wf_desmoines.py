"""
West Fork Des Moines River

JCKM5
ESVI4
EMTI4
RLFI4
HBTI4
FODI4
STRI4
"""

import datetime
import sys

import numpy as np
import pyproj
import pytz
from PIL import Image
from sqlalchemy import text

import pandas as pd
from pyiem.database import get_sqlalchemy_conn
from pyiem.datatypes import distance
from pyiem.network import Table as NetworkTable
from pyiem.plot import MapPlot, nwsprecip
from pyiem.plot.use_agg import plt

p26915 = pyproj.Proj("EPSG:26915")
central = pytz.timezone("America/Chicago")
nt = NetworkTable(["IA_DCP", "MN_DCP"])


def main():
    """Go Main Go"""
    stations = ["MIFI4", "SPWI4", "LNNI4", "CHKI4", "CRRI4", "TURI4"]
    names = [
        "Milford",
        "at Spencer",
        "Linn Grove",
        "Cherokee",
        "Correctionville",
        "Turin",
    ]

    with get_sqlalchemy_conn("hads") as conn:
        df = pd.read_sql(
            text("""
            SELECT distinct station, valid, key, value from raw2024_06 where
            station = ANY(:sids)
            and valid >= '2024-06-21 00:00+00' and
                 valid < '2024-08-24 23:00+00'
            and substr(key, 1, 2) = 'HG' and value > 0 ORDER by valid ASC
            """),
            conn,
            params={"sids": stations},
            index_col=None,
        )
    now = df["valid"].min()
    maxval = df["valid"].max()
    xticks = []
    xticklabels = []
    while now < maxval:
        xticks.append(now)
        lts = now.astimezone(central)
        fmt = "%-I %p" if lts.hour != 0 else "Mid\n%b %d"
        xticklabels.append(lts.strftime(fmt))
        now += datetime.timedelta(hours=3)
    lats = []
    lons = []
    for station, _name in zip(stations, names):
        lats.append(nt.sts[station]["lat"])
        lons.append(nt.sts[station]["lon"])

    bufr = 0.2
    m = MapPlot(
        sector="custom",
        south=(min(lats) - bufr),
        title="23-24 August 2016 Turkey River Gauges",
        subtitle=(
            "Legend shows time delay from initial rise at "
            "Spillville, overall speed to reach location"
        ),
        north=(max(lats) + bufr),
        east=(max(lons) + bufr),
        west=(min(lons) - bufr),
        axisbg="white",
        caption="Data from Iowa Flood Center, USGS, NWS",
    )
    m.ax.set_position([0.57, 0.35, 0.35, 0.55])
    m.cax.set_position([0.93, 0.1, 0.06, 0.8])

    ax = m.fig.add_axes([0.07, 0.35, 0.48, 0.55])
    ids = []
    colors = []
    delay0 = None
    m0 = None
    offset0 = None
    elev0 = None
    for station, name in zip(stations, names):
        mm = p26915(lons[-1], lats[-1])
        if m0 is None:
            m0 = mm
        dist = distance(
            ((mm[0] - m0[0]) ** 2 + (mm[1] - m0[1]) ** 2) ** 0.5, "M"
        ).value("MI")
        ids.append(station)
        df2 = df[df["station"] == station]
        if df2.empty:
            print(f"Missing data for {station}")
            sys.exit()
        # Find first timestamp over 110% of t0
        df3 = df2[df2["value"] > (df2.iloc[0]["value"] * 1.1)]
        if df3.empty:
            print(f"Missing 110% value for {station}")
            sys.exit()
        idx = df2[df2["value"] > (df2.iloc[0]["value"] * 1.1)].index[0]
        delay = df2.loc[idx]["valid"] - df2.iloc[0]["valid"]
        if delay0 is None:
            delay0 = delay
        offset = (delay - delay0).total_seconds() / 3600.0
        if offset == 0:
            offset0 = offset
            speed = 0
        else:
            toffset = offset - offset0
            speed = dist / toffset
        elev = nt.sts[station]["elevation"]
        if elev0 is None:
            elev0 = elev
        elev0 - elev
        elev0 = elev
        label = "%s %s %.1fh %.1fmph" % (station, name, offset, speed)
        ln = ax.plot(
            df2["valid"].values, df2["value"].values, lw=2, label=label
        )
        colors.append(plt.getp(ln[0], "c"))

    ax.legend(loc=(-0.05, -0.5), ncol=2, fontsize=14)
    ax.grid(True)
    ax.set_xticks(xticks)
    ax.set_xticklabels(xticklabels)
    ax.set_ylabel("Stage (ft)")

    img = Image.open("p72h_202406221200.png")
    data = np.flipud(np.asarray(img))
    # 7000,3500 == -130,-60,55,25 ===  -100 to -90 then 38 to 45
    sample = data[1800:2501, 3000:4501]
    sample = np.where(sample == 255, 0, sample)
    data = sample * 0.01
    data = np.where(sample > 100, 1.0 + (sample - 100) * 0.05, data)
    data = np.where(sample > 180, 5.0 + (sample - 180) * 0.2, data)
    lons2 = np.arange(-100, -84.99, 0.01)
    lats2 = np.arange(38, 45.01, 0.01)

    x, y = np.meshgrid(lons2, lats2)
    clevs = [
        0.01,
        0.1,
        0.25,
        0.5,
        0.75,
        1,
        1.5,
        2,
        2.5,
        3,
        3.5,
        4,
        5,
        6,
        8,
        10,
    ]

    m.contourf(x[:, :-1], y[:, :-1], data, clevs, cmap=nwsprecip())

    for lon, lat, myid, c in zip(lons, lats, ids, colors):
        m.plot_values(
            [lon],
            [lat],
            [myid],
            "%s",
            color=c,
            labelbuffer=0,
            outlinecolor="white",
        )
    m.drawcities()
    m.drawcounties()
    m.ax.text(
        0.0,
        -0.1,
        "MRMS 24 Hour Precip ending 7 AM 24 Aug",
        transform=m.ax.transAxes,
    )

    plt.gcf().savefig("240624.png")


if __name__ == "__main__":
    main()
