...
ccursor.execute(
    """SELECT valid, high from climate71 where
 station = 'IA8706' """
)
climate51 = {}
for row in ccursor:
...
ccursor.execute(
    """SELECT day, high from alldata_ia where
...
for yr in range(1900, 2014):
    ...
    if (
        data["%s%02i%02i" % (lts.year, lts.month, lts.day)]
...
for i, yr in enumerate(years):
    if end[i] - start[i] > 15:
        ax.text(
            yr, end[i], "%s" % (yr,), ha="center", va="bottom", rotation=90
        )
...
ax.set_title(
    "1900-2013 Waterloo Streak of High Temp below Average\nwhen day of spring equinox below average as well"
)
