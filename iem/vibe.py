...
def main():
    ...
    ax.set_title(("IEM1 (2005 then 2008 Pontiac Vibe) Gas Mileage + "
    ...
    ax.text(0.5, np.average(y), "%.1f\nAvg" % (np.average(y),),
            color='green', ha='right', va='center', bbox=dict(color='white'))
    y2 = ax.twinx()
    ...
    ax.grid(True)
    y2.set_yticks(np.linspace(y2.get_yticks()[0],
                              y2.get_yticks()[-1],
                              len(ax.get_yticks())))
